...
- test_incus_encryption.sh -> TODO: Add TestIncus_Encryption if needed
...
Note: If test_incus_encryption.sh has unique tests not covered, create TestIncus_Encryption in incus_test.go first. Otherwise, mark encryption testing as future work if out of scope.
  </action>
  <verify>
```bash
...
- All tests migrated from bash to Go
  ...
  <how-to-verify>
Run the test suite 3 times consecutively and verify:
...
Should pass. Then change a help string temporarily and verify test fails.
  ...
  <resume-signal>
Report results:
- "3x pass, no orphans" - Phase complete
