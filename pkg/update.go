...
func (u *SystemUpdater) getUpdatedRootKernelVersion() (string, error) {
	...
	if len(kernelVersions) > 1 {
		// Sort in reverse order to get the newest version
		for i := 0; i < len(kernelVersions)-1; i++ {
			for j := i + 1; j < len(kernelVersions); j++ {
				if kernelVersions[j] > kernelVersions[i] {
					kernelVersions[i], kernelVersions[j] = kernelVersions[j], kernelVersions[i]
				}
			}
		}
	}
